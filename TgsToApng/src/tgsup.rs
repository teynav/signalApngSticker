...
pub fn try_decode(file : &str , location : &str ) {
    ...
    let mut content = vec![];
    bufr.read_to_end(&mut content);
    let mut decoded = GzDecoder::new(&content[..]);
    let mut decres = "".to_owned();
    decoded.read_to_string(&mut decres);
...
pub fn try_tgs_info(file : &str ) -> (u32,u32){
    let mut child = Command::new("tgs2png").arg("-i")
                                                              .arg(&file)
                                                              .stdout(Stdio::piped())
                                                              .spawn().unwrap();
    let output1=child.wait_with_output().expect("Can't read file");

    let mut output = String::from_utf8(output1.stdout.to_vec()).unwrap();   
...
pub fn tgs_to_png(file : String , location : String , threads  : usize , theres : u32 , frames : u32){
    ...
    while counter<frames {
        ...
        pool.execute( move || {
            ...
            let shell_command="tgs2png -s ".to_owned()+&res.to_string()+"x0 -o "+&counter.to_string()+" -n 1  "+&file+" > "+&location+&counter.to_string()+".png";
            let mut child = Command::new(shell);
            child.arg(flag);
            child.arg(shell_command);
            let xx=child.spawn().expect("Failed to run").wait();
        });
        counter=counter+1;
    }
    pool.join();
