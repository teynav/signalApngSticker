...
async fn  download(token : &str , pack : &str) -> Option<(String,u32)> {
    let thisbot=Bot::new(token);
    let down1  = thisbot.get_sticker_set(pack).send().await;
    let down =  match down1 {
        Ok(x) => x,
        Err(_) => {
              return None;
        },
    ...
    return Some ((title.to_owned(),counter));
}

pub fn down(token : &str , pack : &str)-> Option<(String, u32)>{
