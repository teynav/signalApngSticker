...
impl Pngquant {
    ...
    pub fn quantify(&mut self) -> (Vec<u8>, Vec<u8>) {
        let decoding = ImgReader::open(&self.name)
            .unwrap()
            .decode()
            .unwrap()
            .into_rgba8();
        ...
        let mut thispal = vec![];
        for entry in colorp {
            thispal.push(entry.r);
            thispal.push(entry.g);
            thispal.push(entry.b);
            tran.push(entry.a);
        }
    ...
    pub fn bettercompress(&self, sameoutput: bool) {
        ...
        if !sameoutput {
            oxipng::optimize(
                &InFile::Path(PathBuf::from(&self.name)),
                &OutFile::Path(Some(PathBuf::from(&self.newfile))),
                &opt,
            )
            .unwrap();
        } else {
            oxipng::optimize(
                &InFile::Path(PathBuf::from(&self.newfile)),
                &OutFile::Path(None),
                &opt,
            )
            .unwrap();
    ...
    pub fn breakinto(&self, locate: &str) {
        ...
        while counter < totaltimes {
            ...
            crop.save("".to_owned() + locate + "" + &counter.to_string() + ".png")
                .unwrap();
            counter = counter + 1;
        }
    }
...
pub fn compressat(s: &str) {
    ...
    loop {
        ...
        jobs.execute(move || {
            oxipng::optimize(
                &InFile::Path(PathBuf::from(&fname)),
                &OutFile::Path(None),
                &opt,
            )
            .unwrap();
...
pub fn makedirect(s: &str, out: &str, delay: u32) -> f32 {
    ...
    child.wait_with_output().expect("Error Collecting output");
...
pub fn stich(name: &str, _output: &str, _dataw: u32) {
    let mut child = Command::new("apngdis")
        .arg(name)
        .arg("-s")
        .stdout(Stdio::null())
        .spawn()
        .expect("Failed to run the apngdis, exiting");
...
pub fn bypass(scale: u32, frame: u32, pframe: u32, input: String, locate: &str, tempo: String) {
    ...
    let _x = ImgReader::open(&input)
        .unwrap()
        .decode()
        .unwrap()
        .thumbnail(scale, scale * totframe)
        .save(&tempo);
    ...
    while counter < totframe {
        ...
        crop.thumbnail_exact(scale, scale)
            .save_with_format(
                "".to_owned() + locate + "" + &count.to_string() + ".png",
